...
class MatteCard(QFrame):
    def __init__(self, parent=None, accent=False):
        ...
        self.setStyleSheet(
            f"QFrame#{self.objectName()} {{background:{SURFACE}; border:1px solid {border}; border-radius:20px;}}"
        )
...
class StatusPill(QLabel):
    def __init__(self, text, tone="neutral", parent=None):
        ...
        self.setStyleSheet(
            f"QLabel {{color:{fg}; background:{bg}; border:1px solid {ACCENT if tone == 'success' else BORDER}; "
