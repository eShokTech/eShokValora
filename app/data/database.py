...
SCHEMA="""CREATE TABLE IF NOT EXISTS device_models(id TEXT PRIMARY KEY,brand TEXT NOT NULL,name TEXT NOT NULL,year INTEGER,aliases TEXT NOT NULL DEFAULT '',active INTEGER NOT NULL DEFAULT 1);
...
class Database:
    ...
    def list_observations(self,device_key:str)->list[MarketObservation]:
        return self._rows_to_observations(self._fetch_observation_rows("WHERE device_key=?",(device_key,)))
    def list_observations_for_model(self,brand:str,model:str)->list[MarketObservation]:
        prefix=f"{brand.strip().lower()}|{model.strip().lower()}"
        return self._rows_to_observations(self._fetch_observation_rows("WHERE device_key=? OR device_key LIKE ?",(prefix,prefix+"|%")))
    def _fetch_observation_rows(self,where:str="",params:tuple=()):
        with self.connect() as c:return c.execute("SELECT device_key,condition,price,observed_on,source,sample_note,city,sold,confidence,market_type FROM market_observations "+where+" ORDER BY observed_on DESC",params).fetchall()
    @staticmethod
    def _rows_to_observations(rows):
