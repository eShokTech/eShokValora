...
class Device:
    ...
    def key(self) -> str:
        ...
        if self.ram_gb:
            parts.append(f"{self.ram_gb}gb_ram")
        return "|".join(parts)
...
class ValuationInput:
    ...
    def __post_init__(self) -> None:
        ...
        if self.desired_margin_percent is not None and not 0 <= self.desired_margin_percent < 100:
            raise ValueError("El margen deseado debe estar entre 0 y 99.99%.")


@dataclass(frozen=True)
class ValuationResult:
    market_price: float
    total_non_purchase_cost: float
    maximum_purchase_price: float
    recommended_offer: float
    expected_profit_at_max: float
    expected_profit_at_recommended: float
    expected_margin_at_max_percent: float
    expected_margin_at_recommended_percent: float
    viable: bool
