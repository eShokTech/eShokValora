...
class EvidenceState(str,Enum):
    CONFIRMED="confirmed"; VERIFIED="verified"; ESTIMATED="estimated"; UNKNOWN="unknown"; NOT_AVAILABLE="not_available"
...
class DeviceIdentity:
    ...
    carrier:IdentifiedField=IdentifiedField(None,EvidenceState.UNKNOWN)
    functional_test:IdentifiedField=IdentifiedField(None,EvidenceState.UNKNOWN)
    @property
    def incomplete_fields(self)->tuple[str,...]:
        return tuple(n for n,f in (("modelo exacto",self.model),("variante",self.variant),("RAM",self.ram_gb),("almacenamiento",self.storage_gb),("IMEI",self.imei),("operador/red",self.carrier),("prueba funcional",self.functional_test)) if not f.confirmed)
@dataclass(frozen=True)
...
def identity_from_candidate(candidate:IdentificationCandidate)->DeviceIdentity:
    state=EvidenceState.CONFIRMED if candidate.confidence>=1 else EvidenceState.ESTIMATED
    return DeviceIdentity(IdentifiedField(candidate.model.name,state,candidate.source))
def identity_from_variant(model:DeviceModel,variant:DeviceVariant|None=None,*,ram_gb:int|None=None,storage_gb:int|None=None,imei:str|None=None,imei_verified:bool=False,carrier:str|None=None,functional_test_completed:bool=False)->DeviceIdentity:
