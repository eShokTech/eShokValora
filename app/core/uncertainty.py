...
class UncertaintyAssessment:
    reserve:float; score:float; level:str; items:tuple[UncertaintyItem,...]; blocked:bool=False
    @property
    def reasons(self)->tuple[str,...]:return tuple(i.reason for i in self.items)
def assess_uncertainty(*,market_price:float,model_confirmed:bool=True,exact_variant_confirmed:bool=False,ram_confirmed:bool=False,storage_confirmed:bool=False,imei_verified:bool=False,imei_reported:bool=False,carrier_confirmed:bool=True,functional_test_completed:bool=False)->UncertaintyAssessment:
    ...
    if imei_reported:
        items.append(UncertaintyItem("IMEI","El IMEI fue reportado; el equipo no debe valorarse como inventario vendible.",1.0,critical=True))
    elif not imei_verified:
        items.append(UncertaintyItem("IMEI","El IMEI no está verificado; existe riesgo de reporte, blacklist o imposibilidad de venta.",.10,critical=True))
    ...
    blocked=imei_reported
    reserve=market_price if blocked else market_price*min(.35,sum(i.penalty_rate for i in items))
    if not blocked and any(i.critical for i in items):reserve+=market_price*.03
    return UncertaintyAssessment(round(min(reserve,market_price),2),round(score,2),"critical" if blocked else level,tuple(items),blocked)
...
def conservative_variant(variants:tuple[DeviceVariant,...])->DeviceVariant|None:return min(variants,key=lambda v:(min(v.storage_options_gb) if v.storage_options_gb else 0,min(v.ram_options_gb) if v.ram_options_gb else 0,v.model_number)) if variants else None
