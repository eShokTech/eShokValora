...
def value_device(request: ValuationRequest) -> ValuationEngineResult:
    market = estimate_market(request.observations, condition=request.condition)
    risk = assess_risk(
        repair_cost=request.repair_cost, market_price=market.price,
        known_faults=request.known_faults, unknown_faults=request.unknown_faults,
    ...
    uncertainty = assess_uncertainty(
        ...
        imei_verified=identity is None or identity.imei.confirmed,
    )
    valuation = calculate_valuation(ValuationInput(
        market_price=market.price,
        repair_cost=request.repair_cost,
        risk_reserve=risk.reserve + uncertainty.reserve,
    ...
    return ValuationEngineResult(market=market, risk=risk, uncertainty=uncertainty, valuation=valuation)
